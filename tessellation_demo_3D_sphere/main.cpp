...
int main () {
  ...
  */
  ...
  suv.near = 1.0f;
  suv.far = 100.f;
  suv.camera_position = vec3(0.0f, 5.0f, 10.0f);
                            ...
                            ShaderUniformVars& suv)
{
  ...
  if (GLFW_PRESS == glfwGetKey (window, GLFW_KEY_DOWN)) {
    ...
    if (GLFW_PRESS == glfwGetKey (window, GLFW_KEY_W)) {
      if (!w_was_down) {
        w_was_down = true;
        d1->setRotate(d1->getRotateAngle() + 25, 1, 0, 0);
        suv.modelMatrix = d1->getModelMatrix();
        shader.setMat4("modelMatrix", suv.modelMatrix);
    ...
    if (GLFW_PRESS == glfwGetKey (window, GLFW_KEY_E)) {
      if (!e_was_down) {
        e_was_down = true;
        d1->setRotate(d1->getRotateAngle() + 25, 0, 1, 0);
        suv.modelMatrix = d1->getModelMatrix();
        shader.setMat4("modelMatrix", suv.modelMatrix);
    ...
    if (GLFW_PRESS == glfwGetKey (window, GLFW_KEY_R)) {
      if (!r_was_down) {
        r_was_down = true;
        d1->setRotate(d1->getRotateAngle() + 25, 0, 0, 1);
        suv.modelMatrix = d1->getModelMatrix();
        shader.setMat4("modelMatrix", suv.modelMatrix);
